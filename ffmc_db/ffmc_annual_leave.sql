...
CREATE TABLE `annual_leave` (
  `annual_leaveId` smallint(25) NOT NULL,
  `employeeId` smallint(25) NOT NULL,
  `date_from` date DEFAULT NULL,
  `date_to` date DEFAULT NULL,
  `approved` enum('pending','approved','declined') DEFAULT 'pending',
  `type` enum('holiday','sick','religious event','maternity leave','paternity leave','other') DEFAULT 'holiday',
  PRIMARY KEY (`annual_leaveId`)
